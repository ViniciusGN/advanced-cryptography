...
class Group(object):
    ...
    def law(self, g1, g2):
        if(self.l == "ZpAdditive"):
            ...
            m = lab1_utils.deg(self.poly)
            F = Group(l="F2^n", e=1, N=(1 << m) - 1, p=None, poly=self.poly)
            O = IDT_ELEMENT
            P, Q = g1, g2
            if (P == O): 
                return Q
            ...
            if (P == Q):
                # λ = Px + (Py * Px^{-1})
                inv_x1 = F.exp(x1, -1)
                lam = x1 ^ F.law(y1, inv_x1)
                lam2 = F.law(lam, lam)
                x3 = lam2 ^ lam ^ self.A
                y3 = F.law(lam, x1 ^ x3) ^ x3 ^ y1
                return [x3, y3]

            # λ = (Qy + Py) * (Qx + Px)^{-1}
            inv_dx = F.exp(x2 ^ x1, -1)
            lam = F.law(y2 ^ y1, inv_dx)
            lam2 = F.law(lam, lam)
            x3 = lam2 ^ lam ^ x1 ^ x2 ^ self.A
            y3 = F.law(lam, x1 ^ x3) ^ x3 ^ y1
            return [x3, y3]
...
class SubGroup(Group):
    ...
    def verify(self, P):
        ...
        if(P == IDT_ELEMENT):
            ...
            x, y = P
            # left = y^2 + x*y    (sum = XOR; square/mul by F.law)
            left  = F.law(y, y) ^ F.law(x, y)

            # right = x^3 + a*x^2 + b
            x2 = F.law(x, x)          # x^2
            x3 = F.law(x2, x)         # x^3
            right = x3 ^ F.law(self.A, x2) ^ self.B
